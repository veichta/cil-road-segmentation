...
def getTopoLoss(likelihood_tensor, gt_tensor, device, topo_size=100):
    ...
    for y in range(0, likelihood.shape[0], topo_size):
        for x in range(0, likelihood.shape[1], topo_size):
            ...
            if len(pd_lh.shape) < 2:
                # print(f"Skipped due to pd_lh shape is of shape{str(pd_lh.shape)}")
                continue
            if len(pd_gt.shape) < 2:
                # print(f"Skipped due to pd_gt shape is of shape{str(pd_gt.shape)}")
                continue

            force_list, idx_holes_to_fix, idx_holes_to_remove = compute_dgm_force(
                pd_lh, pd_gt, pers_thresh=0.03
            )
