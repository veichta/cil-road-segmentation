...
def getCriticalPoints(likelihood):
    ...
    if (len(pairs_lh[0])==0): return 0, 0, 0
...
def getTopoLoss(likelihood, gt):
    ...
    for y in range(0, likelihood.shape[0], topo_size):
        for x in range(0, likelihood.shape[1], topo_size):
            ...
            # if not(pairs_lh): continue
            # if not(pairs_gt): continue
...
if __name__ == "__main__":
    # Topology Test Images
    circle = 1 - imread('circle.png')
    double_circle = 1 - imread('double_circle.png')
    
    gt = 1 - imread('satimage_0.png')
    lh = 1 - imread('satimage_1.png')
    
    # loss_topo = getTopoLoss(lh, lh)
    # print(loss_topo)
    
    # # Checking the circle only outputs a single output for either birth or death
    print(len(getCriticalPoints(circle)[-2])) # Output = 1
    print(len(getCriticalPoints(double_circle)[-2])) # Output = 2